 ...
 * 
 * COMPONENTE MODIFICADO PARA ATENDER AS NECESSIDADES DO MEC
 * MODIFICADO POR @AUTHOR RAPHAELNEVES
 * MODIFICADO EM 18-10-2016
 * 
 * */
...
//be followed.
    ...
    define( 'visualcaptcha/core',[],function() {
        ...
        _refresh = function( config ) {
            ...
            config.request( startURL, function( response ) {
                ...
                if ( response.audioValido ) {
                    config.audioFieldName = response.audioValido;
                }
                ...
                if ( response.imagemValida ) {
                    config.imageName = response.imagemValida.value;
                    sessionStorage.setItem('captcha', response.imagemValida.value);
                }
                
                // Set the correct image values
                if ( response.imagens ) {
                    config.imageValues = response.imagens;
                }
        ...
        _imageUrl = function( config, i ) {
            ...
            url = "http://localhost:8080/captcha-ws" + config.url + config.routes.image + i;

            return _addUrlParams( config, url, params );
        };

        _audioUrl = function( config, ogg ) {
            var url = "http://localhost:8080/captcha-ws" + config.routes.audio;
    ...
    define( 'visualcaptcha/templates',[],function() {
        ...
        _buttonsHTML = function( captcha, language, path ) {
            ...
            btnAccessibility =
                ...
                '<p style="margin: 0;"><img src="{path}accessibility{retinaExtra}.png" title="{accessibilityTitle}" alt="{accessibilityAlt}" /></p>' +
                '</div>';

            btnRefresh =
                '<div class="visualCaptcha-refresh-button">' +
                '<p style="margin: 0;"><img src="{path}refresh{retinaExtra}.png" title="{refreshTitle}" alt="{refreshAlt}" /></p>' +
                '</div>';
        ...
        _accessibilityHTML = function( captcha, language ) {
        	...
        	var audio = url + "visualcaptcha/assets/audios/" + captcha.audioFieldName().path;
            string =
        ...
        _imagesHTML = function( captcha, language ) {
            ...
            var array = captcha.numberOfImages();
            ...
            for ( var i = 0, l = array.length; i < l; i++ ) {
            	...
            	var imagem = url + "visualcaptcha/assets/images/" + array[i].path;

            	string =
                    '<div class="img">' +
                    '<p style="margin: 0;"><img src='+ imagem +' name='+ array[i].value + ' id="visualCaptcha-img-{i}" data-index="{i}" alt="" title="" /></p>' +
                    '</div>';
    ...
    define( 'visualcaptcha/language',[],function() {
        ...
        return {
            accessibilityAlt: 'Ícone de áudio',
            accessibilityTitle: 'Opção de acessibilidade: escute uma pergunta e responda a ela.',
            accessibilityDescription: 'Digite abaixo a <strong>resposta</strong> para o que você ouviu. Números ou letras.:',
            explanation: 'Selecione o ícone de <strong>ANSWER</strong>',
            refreshAlt: 'Atualizar ícone',
            refreshTitle: 'Atualizar: obtenha novos ícones e opção de acessibilidade.'
        };
    } );
    /*global define */
    ...
    define( 'visualcaptcha.jquery',[
        ...
        _getCaptchaData = function( element ) {
        	...
        	var sessao = sessionStorage.getItem("captcha");
        	var captcha = element.find(".visualCaptcha-selected > p > img").attr("name");
        ...
        $.fn.visualCaptcha = function( options ) {
            ...
            this.on( 'click', '.visualCaptcha-accessibility-button', _toggleAccessibility )
            // Bind refresh button
            this.on( 'click', '.visualCaptcha-refresh-button', _refreshCaptcha )
            // Bind images
            this.on( 'click', '.visualCaptcha-possibilities .img', _chooseImage );
